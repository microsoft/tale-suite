...
def build_prompt(observation, history, question, qa_history):
    messages = [
        {
            "role": "system",
            "content": "You are playing a text-based game. Use short phrases to interact with the game, e.g. get lamp. When you are stuck, try using the help command.",
        },
    ]
...
class ReactAgent(twbench.Agent):
    ...
    def act(self, obs, reward, done, infos):
        ...
        response = self.conversation.prompt(
            prompt, system=self.system_prompt, temperature=0.0, seed=self.seed, top_p=1
        )
